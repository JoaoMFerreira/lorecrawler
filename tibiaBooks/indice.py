...
DATA = []
with open("/home/joao-marcos/myWorkspace/tibiaBooks/tibiaBooks/booksRaw.txt") as f:
    for line in f:
...
def query(index, query, operator='AND', fields=None):
    ...
    for doc_id, score in ids.most_common():
        print('%s found with score of %s' % (bold(DATA[doc_id]['name']), bold(score)))
        print(DATA[doc_id]['text'])
    print('\n')
